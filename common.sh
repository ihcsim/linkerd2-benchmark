...
function install_istio_stress_test_mode() {
  echo "Installing istio (stress test mode)"
  kubectl create ns ${NAMESPACE_ISTIO}
  kubectl -n ${NAMESPACE_ISTIO} apply -f ${RESOURCE_FILE_ISTIO_STRESS_TEST}
  sleep 180
  kubectl -n ${NAMESPACE_ISTIO} delete rules stdio stdiotcp
}
